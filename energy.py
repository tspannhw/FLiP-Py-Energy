...
try:
    while True:
        for dev in Discover.discover().values():
            ...
            electricRec.current_consumption = float(dev.current_consumption())
            electricRec.alias = dev.alias
            electricRec.devicetime = dev.time.strftime('%m/%d/%Y %H:%M:%S')
            electricRec.ledon = dev.led
            end = time.time()
            ...
            #print(json_string)
            # producer.send((json_string).encode('utf-8'),partition_key=str(uuid2))
